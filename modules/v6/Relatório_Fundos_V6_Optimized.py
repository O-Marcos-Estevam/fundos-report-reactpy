...
class ReportDiarioFundosV6:
    ...
    def coletar_dados_completos(self):
        ...
        # 3. Queries MAPS e QORE em PARALELO
        if self.config['performance'].get('parallel_queries', True):
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                future_maps = executor.submit(
                    ...
                    self.input_date,
                    self.config['fundos']['fidcs_maps'],
                    dias_uteis
                )

                future_qore = executor.submit(
                    self.db_manager.get_qore_batch,
                    self.input_date,
                    self.config['fundos']['fundos_qore'],
                    dias_uteis
                )

                # Aguardar resultados
                dados_maps = future_maps.result()
                dados_qore = future_qore.result()
